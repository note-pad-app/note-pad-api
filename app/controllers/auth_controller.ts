...
export default class AuthController {
  ...
  async forgotPassword({ request }: HttpContext) {
    ...
    let user = await User.query().where({ username: uids }).orWhere({ email: uids }).firstOrFail()
    ...
    await mail.send((message) => {
      message
        ...
        .html(
          `<div style="text-align: center"><h2>Password Reset</h2><p>Click the link below to reset your password:</p><a href="http://localhost:5173/reset-password?user_id=${user.id}&token=${token.hash}" style="margin-top: 1rem;background: blue; padding: 10px; border-radius: 2px; color: white">Reset Password</a></div>`
        )
    })
  ...
  async changePassword({ auth, request, response }: HttpContext) {
    ...
    response.status(200).json({ message: 'password is updated successfully' })
  }
}
